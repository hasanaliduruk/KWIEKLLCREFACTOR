...
def button(canvas2, button):
    ...
    if button == button_1:
        ...
        canvas2.unbind_all('<MouseWheel>')
        render_expration_view(canvas, canvas2, window, color, line_color, canvas2_text_color, main_frame_resize)
    if button == button_2:
        window.unbind("<Configure>")
    ...
    if button == button_9:
        ...
        render_updater_view(canvas2, color, window, line_color, canvas2_text_color, CURRENT_VERSION)
    if button == button_10:
    ...
    def check_version_at_startup():
        if check_internet():
            ...
            if data and data['tag_name'] > CURRENT_VERSION:
                version.config(text=f"{CURRENT_VERSION} yeni version({data['tag_name']}) mevcut!", fg='yellow')
                messagebox.showinfo("Güncelleme", f"Yeni bir versiyon mevcut: {data['tag_name']}")
