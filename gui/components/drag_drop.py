...
def drag_drop(row1,row,column,dict_name,text,parent, window, canvas2, color, text_color, dosyalar_dictionary, win=0, bg_image=0, file_image=0, file_type=".xlsx", padx=25, pady=25,):
    if win == 0:
        win = window
    ...
    surukle_text = Label(parent, text=text, background=color, fg=text_color, font=("JetBrainsMonoRoman Regular", 12))
        ...
        for i in file_path:
            ...
            if i != '':
                ...
                if i not in dosyalar_dictionary[dict_name] and k == 1:
                    ...
                    button = tk.Button(buttons_frame, text=i, font=("JetBrainsMonoRoman Regular", 15 * -1), height=1, border= 0, anchor='w')
                    ...
                    button_tik = tk.Button(buttons_frame,image=image_dictionary['sil_button_image'], border=0, width=35, command=lambda b= button, db = None, bl = None: delete_button_func(b, db, bl))
    ...
    scrollbar_v = tk.Scrollbar(main_canvas, orient=tk.VERTICAL, command=drop_canvas.yview)
    scrollbar_v.grid(column = 1, row = 0, sticky='ns')
    drop_canvas.config(yscrollcommand=scrollbar_v.set)

    scrollbar_h = tk.Scrollbar(main_canvas, orient=tk.HORIZONTAL, command=drop_canvas.xview)
...
def ham_drag_drop2(row1,row,column,dict_name,text,parent, 
                   window, canvas2, dosyalar_dictionary, color, text_color):
    dosyalar_dictionary[dict_name] = []
    button_list = []
    ...
    surukle_text = Label(parent, text=text, background=color, fg=text_color,)
            ...
            if i != '':
                ...
                if i not in dosyalar_dictionary[dict_name] and k == 1:
                    ...
                    def tasi(lst, from_index, to_index):
                        ...
                        update_buttons()
                    button_tik = tk.Button(buttons_frame,image=sil_button_image, border=0, width=35, command=lambda b= button, db = None, bl = None: delete_button_func(b, db, bl))
                    button_tik.config(command=lambda bf = buttons_frame, b=button, bl=button_list: delete_button_func(bf, b, bl))
