...
class MyScrollbar(tk.Canvas):
    def __init__(self, parent, target,command, orient='vertical', thickness=14, thumb_thickness=10, background_color='black', thumb_color='white', line_color='gray', **kwargs):
        super().__init__(parent, background=background_color, border=0, highlightthickness=0, **kwargs)
        ...
        self.up_arrow = AnimatedImage(
            self,
            bg=background_color,
            image_path=self.up_arrow_image,
            cursor='hand2'
        )
        self.down_arrow = AnimatedImage(
            self,
            bg=background_color,
            image_path=self.down_arrow_image,
            cursor='hand2'
        )
    ...
    def draw_rectangle(self, x, y, width, height, fill='white'):
        item = self.create_rectangle(x, y, float(x)+float(width), float(y)+float(height), fill=fill, outline='')
        return item
    ...
    def button_motion(self, event):
        ...
        if event.y-self.starty > self.pad:
            if event.y-self.starty+self.pad < self.winfo_height():
                a = float((event.y-self.starty-self.pad)/(self.winfo_height()-2*self.pad))
