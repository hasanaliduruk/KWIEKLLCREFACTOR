...
class CustomOptionMenu(tk.Frame):
    def __init__(
        ...
        for option in options:
            ...
            button.pack(fill="x")
            button.bind(
                "<Enter>", lambda e: self.on_enter(e, on_enter_color, on_enter_fg)
            )
            button.bind(
                "<Leave>",
                lambda e: self.on_enter(e, background_color, foreground_color),
            )
            if option != options[-1]:
                line.pack(fill="x")
    ...
    def updater(self, options):
        ...
        for option in options:
            ...
            button.bind(
                "<Enter>",
                lambda e: self.on_enter(e, self.on_enter_color, self.on_enter_fg),
            )
            button.bind(
                "<Leave>",
                lambda e: self.on_enter(
                    e, self.background_color, self.foreground_color
                ),
            )
            if option != options[-1]:
                line.pack(fill="x")
    ...
    def toggle(self):
        if self.visible:
            ...
            self.place(
                x=self.parent_button.winfo_rootx() - x,
                y=self.parent_button.winfo_rooty()
                + self.parent_button.winfo_height()
                - y,
                width=self.parent_button.winfo_width(),
            )
    ...
    def on_click_outside(self, event):
        if self.visible:
            widget = event.widget
            if widget != self.parent_button and widget.winfo_containing(
