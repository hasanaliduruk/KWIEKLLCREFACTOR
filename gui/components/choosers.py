...
class PathAdressGroup(tk.Frame):
    def __init__(
        self, parent, window, bg_color, text_color, line_color, *args, **kwargs
    ):
        super().__init__(
            parent,
            background=bg_color,
        ...
        self.inner_path_frame = tk.Frame(self, background=bg_color, height=30)
        ...
        self.browse_button.bind(
            "<Button-1>",
            lambda e: self.browse_click(e, "#8AB4F8", "black", self.save_path),
        )
        self.browse_button.bind(
            "<ButtonRelease-1>",
            lambda e: self.browse_color_change(e, "#727478", "white"),
        )
        self.browse_button.bind(
            "<Enter>", lambda e: self.browse_color_change(e, "#727478", text_color)
        )
        self.browse_button.bind(
            "<Leave>", lambda e: self.browse_color_change(e, line_color, "white")
        )
        ...
        self.save_path.bind(
            "<Button-1>",
            lambda e: on_focus_in(e, self.save_path, placeholder, text_color),
        )
        self.save_path.bind(
            "<FocusOut>",
            lambda e: on_focus_out(e, self.save_path, placeholder, text_color),
        )
        window.bind(
            "<Button-1>",
            lambda e: on_click_outside(e, self.save_path, placeholder, text_color),
        )
