...
def render_restock_view(
    ...
    def on_resize(e):
        ...
        if (
            canvas2.winfo_width()
            < resize_dictionary[restock_inner_frame]["width"] * scale
        ):
        ...
        if p + 55 > canvas2.winfo_height():
            ...
            restock_inner_frame.configure(
                height=canvas2.winfo_height(), width=frame_width
            )
    ...
    def updater():
        ...
        if active_dictionary["export"] == 1:
            export_main_canvas.grid(
                column=0, row=5, columnspan=2, sticky="nwes", padx=25
            )
            export_surukle_text.grid(
                column=0, row=4, columnspan=2, padx=25, pady=10, sticky="w"
            )

        if active_dictionary["restock"] == 1:
            restock_main_canvas.grid(
                column=0, row=7, columnspan=2, sticky="nwes", padx=25
            )
            restock_surukle_text.grid(
                column=0, row=6, columnspan=2, padx=25, pady=10, sticky="w"
            )

        alt_canvas.config(
            height=restock_settings.winfo_y() + restock_settings.winfo_height() + 100
        )
        alt_canvas.update()
    ...
    restock_inner_frame = Frame(
        canvas2, bg=color, height=canvas2.winfo_height(), width=0
    )
    ...
    restock_main_scrollbar = MyScrollbar(
        window,
        target=canvas2,
    ...
    width = int(650)
    ham_liste = ham_drag_drop2(
        row1=0,
        row=1,
        column=0,
        dict_name="ham_dosyalar_liste",
        text="Ham dosyalarin excellerini asagiya surukleyip birakiniz:",
        parent=alt_canvas,
        window=window,
        canvas2=canvas2,
        color=color,
    ...
    export_liste = drag_drop(
        row1=2,
        row=3,
        column=0,
        dict_name="export_dosyalar_liste",
        text="Export dosyalarin excellerini asagiya surukleyip birakiniz:",
        parent=alt_canvas,
        window=window,
        canvas2=canvas2,
        color=color,
    ...
    restock_liste = drag_drop(
        row1=4,
        row=5,
        column=0,
        dict_name="restock_dosyalar_liste",
        text="Restock excelini asagiya surukleyip birakiniz:",
        parent=alt_canvas,
        window=window,
        canvas2=canvas2,
        color=color,
    ...
    restock_option_frame = Frame(options_frame, bg=line_color, height=70)
    ...
    line_1 = Frame(options_frame, height=2, bg="#79918B")
    line_1.grid(column=0, row=1, sticky="we")

    export_option_frame = Frame(options_frame, bg=line_color, height=70)
    ...
    welcome_line = Frame(ust_canvas, background=line_color, height=2)
    file_path = Label(
        ust_canvas,
    ...
    browse_button.bind(
        "<Button-1>",
        lambda e: browse_click(e, "#8AB4F8", "black", save_path_text, browse_button),
    )
    browse_button.bind(
        "<ButtonRelease-1>",
    ...
    save_path_text.bind(
        "<Button-1>",
        lambda e: on_focus_in(e, save_path_text, placeholder, canvas2_text_color),
    )
    save_path_text.bind(
        "<FocusOut>",
        lambda e: on_focus_out(e, save_path_text, placeholder, canvas2_text_color),
    )
    window.bind(
        "<Button-1>",
        lambda e: on_click_outside(e, save_path_text, placeholder, canvas2_text_color),
    )
    ...
    def print_liste(restock_settings, path):
        ...
        if path != "Example: C:/Users/Username/Desktop/sonuc":
            ...
            def run_in_thread():
                try:
                    ...
                    window.after(
                        0,
                        lambda: text_print(
                            restock_output, result["message"], color="#90EE90"
                        ),
                    )
                    window.after(
                    ...
                    window.after(
                        0,
                        lambda: text_print(
                            restock_output, f"Hata: {str(e)}", color="red"
                        ),
                    )
            ...
            text_print(
                restock_output,
                "Hata: Dosya yolunu doğru girdiğinizden emin olun ve tekrar deneyin...",
                color="red",
            )
    ...
    restock_submit_button.bind(
        "<Button-1>",
        lambda e: print_liste(restock_settings, save_path_text.get(1.0, END)),
    )
    restock_submit_button.bind(
        "<ButtonRelease-1>",
        lambda e: color_change(e, "#727478", "white", restock_submit_button),
    )
    restock_submit_button.bind(
        "<Enter>",
        lambda e: color_change(e, "#727478", canvas2_text_color, restock_submit_button),
    )
    restock_submit_button.bind(
        "<Leave>", lambda e: color_change(e, line_color, "white", restock_submit_button)
    )

    future_price_button.bind(
        "<Button-1>",
        lambda e: render_futureprice_view(
            future_price_button,
            canvas2,
            window,
            color,
    ...
    future_price_button.bind(
        "<ButtonRelease-1>",
        lambda e: color_change(e, "#727478", "white", future_price_button),
    )
    future_price_button.bind(
        "<Enter>",
        lambda e: color_change(e, "#727478", canvas2_text_color, future_price_button),
    )
    future_price_button.bind(
        "<Leave>", lambda e: color_change(e, line_color, "white", future_price_button)
    )
    ...
    # window.update_idletasks()
    updater()
    resize_dictionary[restock_inner_frame] = {
        "width": canvas2.winfo_width(),
        "height": restock_inner_frame.winfo_height(),
    }
    resize_dictionary[alt_canvas] = {
        "width": alt_canvas.winfo_width(),
        "height": alt_canvas.winfo_height(),
        "x": alt_canvas.winfo_x(),
        "y": alt_canvas.winfo_y(),
    }
