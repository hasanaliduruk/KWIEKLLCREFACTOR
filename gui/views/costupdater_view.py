...
def render_costupdater_view(
    ...
    def new_active():
        if "costupdater2_settings.txt" not in os.listdir("Settings"):
            write_settings(
                "Settings/costupdater2_settings.txt", costupdater2_settings_var
            )
    ...
    def new_deactive():
        if "costupdater_settings.txt" not in os.listdir("Settings"):
            write_settings(
                "Settings/costupdater_settings.txt", costupdater_settings_var
            )
    ...
    canvas2.config(
        yscrollcommand=costupdater_scrollbar.set, scrollregion=canvas2.bbox("all")
    )
    ...
    top_canvas = Canvas(
        cost_main_frame, background=color, highlightthickness=0, border=0
    )
    ...
    # widgets:
    title_frame = Frame(top_canvas, background=color)
    title = Label(
        title_frame,
        background=color,
    ...
    def output2(path):
        ...
        if not csv_files:
            text_print(
                output_text, "Hata: İşlenecek CSV dosyası sürüklemediniz.", color="red"
            )
            return
