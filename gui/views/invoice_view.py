...
def render_invoice_view(
    ...
    canvas2.config(
        yscrollcommand=invoice_scrollbar.set, scrollregion=canvas2.bbox("all")
    )
    invoice_scrollbar.pack(side=RIGHT, fill=tk.Y)
    top_canvas = Canvas(
        invoice_main_frame, border=0, highlightthickness=0, background=color
    )
    ...
    invoice_title_line = Frame(top_canvas, height=2, background=line_color)
    ...
    def output(path):
        ...
        def run_in_thread():
            try:
                ...
                output_text.after(
                    0,
                    lambda: open_folder_in_explorer(
