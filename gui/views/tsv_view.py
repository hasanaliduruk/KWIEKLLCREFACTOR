...
def render_tsv_view(
    ...
    tvs_main_frame = Frame(canvas2, bg=color, height=canvas2.winfo_height(), width=750)
    ...
    ust_canvas = Canvas(tvs_main_frame, border=0, highlightthickness=0, bg=color)
    ...
    tsv_title_line = Frame(
        ust_canvas, height=2, bg=line_color, border=0, highlightthickness=0
    )
    ...
    tsv_browse_button.bind(
        "<Button-1>",
        lambda e: browse_click(e, "#8AB4F8", "black", tsv_path_text, tsv_browse_button),
    )
    tsv_browse_button.bind(
        "<ButtonRelease-1>",
        lambda e: browse_color_change(e, "#727478", "white", tsv_browse_button),
    )
    tsv_browse_button.bind(
        "<Enter>",
        lambda e: browse_color_change(
            e, "#727478", canvas2_text_color, tsv_browse_button
        ),
    )
    tsv_browse_button.bind(
        "<Leave>",
        lambda e: browse_color_change(e, line_color, "white", tsv_browse_button),
    )
    save_button.bind(
    ...
    tsv_path_text.bind(
        "<Button-1>",
        lambda e: on_focus_in(e, tsv_path_text, placeholder, canvas2_text_color),
    )
    tsv_path_text.bind(
        "<FocusOut>",
        lambda e: on_focus_out(e, tsv_path_text, placeholder, canvas2_text_color),
    )
    window.bind(
        "<Button-1>",
        lambda e: on_click_outside(e, tsv_path_text, placeholder, canvas2_text_color),
    )

    tvs_bg_image = PhotoImage(file=relative_to_assets("tvs_bg_rs.png"))
    tvs_file_image = PhotoImage(file=relative_to_assets("TVS_file.png"))

    tvs_drop_return = drag_drop(
        row1=0,
        row=1,
        column=0,
        dict_name="tsv",
        text=".tsv uzantili dosyalarinizi asagiya surukleyip birakiniz...",
        parent=alt_canvas,
        file_image=tvs_file_image,
        bg_image=tvs_bg_image,
        file_type=".tsv",
        padx=(25, 0),
        window=window,
        canvas2=canvas2,
    ...
    tsv_settings_text.grid(
        column=0, row=3, columnspan=2, sticky="we", padx=(25, 0), pady=5
    )
    ...
    def output(path):
        ...
        if not tsv_files:
            text_print(
                tsv_output, "Hata: İşlenecek TSV dosyası sürüklemediniz.", color="red"
            )
            return
        ...
        def run_in_thread():
            try:
                ...
                tsv_output.after(
                    0,
                    lambda: text_print(tsv_output, result["message"], color="#90EE90"),
                )
                tsv_output.after(0, lambda: open_folder_in_explorer(path))
            except Exception as e:
                tsv_output.after(
                    0, lambda: text_print(tsv_output, f"Hata: {str(e)}", color="red")
                )
