...
def parse_order_settings(settings_content: str):
    sutunlar_dict = {
        'restock_upc': [], 'restock_pcs': [], 'restock_suplier': [], 'restock_notes': [],
        'orderform_upc': [], 'orderform_pcs': [], 'orderform_suplier': [],
    }
    ...
    for line in lines:
        ...
        if '=' in line:
            ...
            if a == 0:  # RESTOCK
                ...
                elif key == 'notes': sutunlar_dict['restock_notes'] = vals
            elif a == 1:  # ORDER FORM
...
def process_order_create(restock_files: list, orderform_files: list, template_path: str, output_folder: str, settings_content: str, progress_callback=None) -> dict:
    ...
    # 1. RESTOCK İŞLEMLERİ
    if progress_callback: progress_callback("Restock excel dosyası okunuyor...")
    ...
    for i, a in enumerate(upc_values):
        if pcs_values[i] != 0:
            ...
            if sup not in output_dictionary: output_dictionary[sup] = {}
            if a not in output_dictionary[sup]: output_dictionary[sup][a] = 0
            output_dictionary[sup][a] += pcs_values[i]
            
            note = notes_values[i]
            if note != 0:
                if note not in output_dictionary: output_dictionary[note] = {}
                if a not in output_dictionary[note]: output_dictionary[note][a] = 0
                output_dictionary[note][a] += pcs_values[i]
                
    # 2. ORDER FORM İŞLEMLERİ
    if progress_callback: progress_callback("Order Form excel dosyası okunuyor...")
    ...
    if progress_callback: progress_callback("Bulunan değerler template dosyalarına yazdırılıyor...")
    ...
    return {"status": "success", "message": "Order Create işlemi başarıyla tamamlandı!", "output_path": target_dir}