...
def ensure_settings():
    ...
    if not os.path.exists(settings_path):
        with open(settings_path, "w", encoding="utf-8") as file:
            file.write(
                "login_button_id = mainForm:j_idt23, mainForm:j_idt13, mainForm:j_idt22\n"
    ...
    for target_id, skus in main_dictionary.items():
        for sku, data in skus.items():
            ...
            ws.cell(row=a, column=6).value = (
                data.get("date")[0] if data.get("date") else ""
            )
            ws.cell(row=a, column=7).value = data.get("noktali", "")
...
def process_expiration(
    ...
    if progress_callback:
        progress_callback("Giriş başarılı. Shipment verileri çekiliyor...", "#90EE90")
    ...
    for target_id in id_list:
        ...
        if not href:
            if progress_callback:
                progress_callback(
                    f"Uyarı: '{target_id}' için sevkiyat linki bulunamadı.", "yellow"
                )
        id_dict[target_id] = href

    for target_id in id_list:
        if not id_dict[target_id]:
            continue
        ...
        for tr in trler:
            ...
            main_dict[target_id][sku] = {
                ...
                "shipped": (
                    tr.findAll("td")[3].text if len(tr.findAll("td")) > 3 else ""
                ),
                "created": fba_date,
        ...
        if progress_callback:
            progress_callback(
                f"{target_id} için {len(main_dict[target_id])} adet ürün bulundu, işleniyor...",
                "white",
            )
        javax = soup.find("input", {"name": "javax.faces.ViewState"})["value"]
        ...
        for a, sku in enumerate(skus):
            if progress_callback:
                progress_callback(
                    f"İşleniyor: {target_id} -> Ürün {a+1}/{len(skus)}", "white"
                )

            payload = {
                "mainForm": "mainForm",
                "javax.faces.ViewState": javax,
                "mainForm:shipmentItems_instantSelectedRowKey": main_dict[target_id][
                    sku
                ]["item_id"],
                "mainForm:shipmentItems_selection": main_dict[target_id][sku][
                    "item_id"
                ],
            ...
            main_dict[target_id][sku]["noktali"] = (
                format_date(unique_dates[0]) if unique_dates else None
            )

        if progress_callback:
            progress_callback(f"'{target_id}' Excel'e yazılıyor...", "white")
        write_excel(main_dict, target_id, output_path)

    if progress_callback:
        progress_callback("Combined Excel dosyası yazılıyor...", "white")
    write_combined_excel(main_dict, output_path)

    return {
        "status": "success",
        "message": "Expiration işlemi başarıyla tamamlandı!",
