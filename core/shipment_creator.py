        ...
        if "=" in line:
            ...
            if section == 0:
                if key == "upc":
                    ...
                    sutunlar_dict["restock_asin"] = vals
                elif key == "pk":
                    sutunlar_dict["restock_pk"] = vals
                elif key == "price":
                    sutunlar_dict["restock_price"] = vals
                elif key == "suplier":
                ...
                if key == "upc":
                    ...
                    sutunlar_dict["orderform_asin"] = vals
                elif key == "sku":
                    sutunlar_dict["orderform_sku"] = vals
                elif key == "pk":
                    sutunlar_dict["orderform_pk"] = vals
                elif key == "price":
                    sutunlar_dict["orderform_price"] = vals
                elif key == "suplier":
                    sutunlar_dict["orderform_suplier"] = vals
            elif section == 2:
                if key == "shipquantity":
                    sutunlar_dict["invoice_shipquantity"] = vals
                elif key == "upc":
                    sutunlar_dict["invoice_upc"] = vals
                elif key == "price":
                    sutunlar_dict["invoice_price"] = vals
                elif key == "packsize":
                    sutunlar_dict["invoice_packsize"] = vals
                elif key == "brand":
                    sutunlar_dict["invoice_brand"] = vals
                elif key == "description":
                    sutunlar_dict["invoice_description"] = vals
...
def get_col(df, possible_cols, context):
    for col in possible_cols:
        if col in df.columns:
            return col
    raise ValueError(
        f"Eksik Sütun: {context} için beklenen sütunlardan hiçbiri bulunamadı. Beklenenler: {possible_cols}"
    )
...
def process_shipment_creation(
    ...
    if not invoice_files or not order_form_files or not restock_files:
        raise ValueError(
            "Hata: Gerekli kaynak dosyalardan biri (Invoice, Order Form veya Restock) eksik."
        )
    ...
    if progress_callback:
        progress_callback("Invoice dosyası okunuyor...")
    df_inv = pd.read_excel(invoice_files[0])
    invoice_form_dict = {
        "ShipQuantity": df_inv[
            get_col(
                df_inv, sutunlar_dict["invoice_shipquantity"], "Invoice ShipQuantity"
            )
        ].tolist(),
        "Upc": df_inv[
            get_col(df_inv, sutunlar_dict["invoice_upc"], "Invoice UPC")
        ].tolist(),
        "Price": df_inv[
            get_col(df_inv, sutunlar_dict["invoice_price"], "Invoice Price")
        ].tolist(),
        "PackSize": df_inv[
            get_col(df_inv, sutunlar_dict["invoice_packsize"], "Invoice PackSize")
        ].tolist(),
        "Brand": df_inv[
            get_col(df_inv, sutunlar_dict["invoice_brand"], "Invoice Brand")
        ].tolist(),
        "Description": df_inv[
            get_col(df_inv, sutunlar_dict["invoice_description"], "Invoice Description")
        ].tolist(),
    }

    # 2. ORDER FORM OKUMA
    if progress_callback:
        progress_callback("Order Form dosyası okunuyor...")
    df_ord = pd.read_excel(order_form_files[0])
    order_form_dict = {
        "Upc": df_ord[
            get_col(df_ord, sutunlar_dict["orderform_upc"], "OrderForm UPC")
        ].tolist(),
        "Price": df_ord[
            get_col(df_ord, sutunlar_dict["orderform_price"], "OrderForm Price")
        ].tolist(),
        "Suplier": df_ord[
            get_col(df_ord, sutunlar_dict["orderform_suplier"], "OrderForm Suplier")
        ].tolist(),
    }
    for i in range(1, len(sutunlar_dict["orderform_asin"]) + 1):
        col_name = (
            sutunlar_dict["orderform_pcs"][0]
            if i == 1
            else f"{sutunlar_dict['orderform_pcs'][0]}.{i-1}"
        )
        order_form_dict[f"Pcs {i}"] = df_ord[
            get_col(df_ord, [col_name], f"OrderForm PCS {i}")
        ].tolist()
    for a, name in enumerate(sutunlar_dict["orderform_asin"], 1):
        order_form_dict[f"Asin {a}"] = df_ord[
            get_col(df_ord, [name], f"OrderForm ASIN {a}")
        ].tolist()
    for a, name in enumerate(sutunlar_dict["orderform_sku"], 1):
        order_form_dict[f"ASIN{a}_SKU"] = df_ord[
            get_col(df_ord, [name], f"OrderForm SKU {a}")
        ].tolist()
        liste = [
            (
                str(i).split("_")[2]
                if isinstance(i, str) and str(i).count("_") >= 3
                else "#YOK"
            )
            for i in order_form_dict[f"ASIN{a}_SKU"]
        ]
    ...
    restock_form_dict = {
        "Asin": df_res[
            get_col(df_res, sutunlar_dict["restock_asin"], "Restock ASIN")
        ].tolist(),
        "Upc": df_res[
            get_col(df_res, sutunlar_dict["restock_upc"], "Restock UPC")
        ].tolist(),
        "Pcs": df_res[
            get_col(df_res, sutunlar_dict["restock_pcs"], "Restock PCS")
        ].tolist(),
        "PK": df_res[
            get_col(df_res, sutunlar_dict["restock_pk"], "Restock PK")
        ].tolist(),
        "Price": df_res[
            get_col(df_res, sutunlar_dict["restock_price"], "Restock Price")
        ].tolist(),
        "Suplier": df_res[
            get_col(df_res, sutunlar_dict["restock_suplier"], "Restock Suplier")
        ].tolist(),
    }

    # 4. EŞLEŞTİRME (MATCH)
    if progress_callback:
        progress_callback("UPC değerleri eşleniyor...")
    dictionary = {
        k: []
        for k in [
            "UPC",
        ...
        if restock_kontrol:
            ...
            for index in indices:
                ...
                if not (isinstance(Pcs, float) and math.isnan(Pcs)):
                    ...
                    dictionary["SKU2"].append(
                        f"{dc_code}_{upcstr}_{Pk}_{format(Pkint * Price, '.2f')}"
                        if Pkint != "#YOK"
                        else "#YOK"
                    )
            ...
            for index in indices:
                ...
                while True:
                    try:
                        ...
                        Pkint = (
                            int(str(PK).replace("PK", "")) if PK != "#YOK" else "#YOK"
                        )

                        if pd.notna(Asin):
                            dictionary["UPC"].append(upc)
                            dictionary["Price"].append(Price)
                            dictionary["Price Check"].append(
                                order_form_dict["Price"][index]
                            )
                            dictionary["Suplier"].append(
                                order_form_dict["Suplier"][index]
                            )
                            dictionary["ShipQuantity"].append(ShipQuantity)
                            dictionary["PackSize"].append(PackSize)
                            dictionary["Brand"].append(Brand)
                            ...
                            dictionary["Yeni Pcs"].append(0)
                            dictionary["PK"].append(PK)
                            dictionary["SKU"].append(
                                order_form_dict[f"ASIN{a}_SKU"][index]
                            )
                            dictionary["SKU2"].append(
                                f"{dc_code}_{upcstr}_{PK}_{format(Pkint * Price, '.2f')}"
                                if Pkint != "#YOK"
    ...
    # 5. STOCK ALLOCATER
    if progress_callback:
        progress_callback("Stoklar dağıtılıyor...")
    complated_upc = []
    for upc in dictionary["UPC"]:
        if upc not in complated_upc:
            ...
            for index in index_list:
                ...
                if dictionary["Pcs"][index] != "#YOK" and not pd.isna(
                    dictionary["Pcs"][index]
                ):
            ...
            for index in index_list:
                if (
                    dictionary["Pcs"][index] != "#YOK"
                    and not pd.isna(dictionary["Pcs"][index])
                    and pcs > 0
                ):
                    new_pcs = round(
                        float(dictionary["Pcs"][index]) / pcs * float(ShipQuantity)
                    )
    ...
    return {
        "status": "success",
        "message": "Shipment Create işlemi başarıyla tamamlandı!",
