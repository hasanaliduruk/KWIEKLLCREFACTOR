...
def find_column(df, possible_columns, error_context):
    for col in possible_columns:
        if col in df.columns:
            return col
    raise ValueError(
        f"Eksik Sütun Hatası: '{error_context}' dosyasında beklenen sütunlardan hiçbiri bulunamadı. Beklenenler: {possible_columns}. Lütfen dosyayı veya ayarları düzeltin."
    )
...
def process_invoice(
    ...
    if progress_callback:
        progress_callback("Veriler birleştiriliyor...")
    df_merged = pd.concat(dataframes, ignore_index=True)

    if progress_callback:
        progress_callback("Gereksiz sütunlar temizleniyor...")
    df_merged.drop(
        columns=columns_dict["remove"], axis=1, inplace=True, errors="ignore"
    )
    ...
    if progress_callback:
        progress_callback("Tarihler formatlanıyor...")
    ...
    if progress_callback:
        progress_callback("Sayı formatları (nokta/virgül) dönüştürülüyor...")
