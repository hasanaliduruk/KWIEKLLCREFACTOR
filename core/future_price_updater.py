...
def process_future_price(path: str, name: str, restock_excel: str, future_excel: str, progress_callback=None) -> dict:
    ...
    for i, asin in enumerate(all_asins):
        restock_dictionary[asin] = {
            'Price': restock_df['Price'][i] if 'Price' in restock_df.columns else '#YOK',
            'Maliyet': restock_df['Maliyet'][i] if 'Maliyet' in restock_df.columns else '#YOK'
        }
        for col_name in price_columns_list:
            restock_dictionary[asin][col_name] = restock_df[col_name][i]

    if progress_callback: progress_callback("Future Price dosyası okunuyor ve veriler işleniyor...")
    ...
    for i, asin in enumerate(future_asins):
        future_dictionary[asin] = {
            ...
            'Maliyet': future_df['Maliyet'][i] if 'Maliyet' in future_df.columns else '#YOK'
        }
    ...
    for asin in restock_dictionary.keys():
        if asin in future_dictionary:
            ...
            for col_name in price_columns_list:
                ...
                restock_dictionary[asin][future_name] = future_dictionary[asin].get(col_name, '#YOK')
        else:
    ...
    future_price_list = []
    future_maliyet_list = []
    write_dict = {col.lower().replace('price', 'future price'): [] for col in price_columns_list}
    ...
    if progress_callback: progress_callback("Hesaplanan veriler ana tabloya entegre ediliyor...")
    ...
    restock_df.insert(maliyet_index, 'Future Price', future_price_list, allow_duplicates=False)
    restock_df.insert(maliyet_index + 1, 'Future Maliyet', future_maliyet_list, allow_duplicates=False)
    ...
    if dont_exist_list:
        price_indices = [restock_df.columns.get_loc(col) for col in restock_df.columns if 'price' in col.lower()]
