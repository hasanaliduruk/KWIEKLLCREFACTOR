...
def read_settings():
    sutun_dictionary = {
        'upc_sutunlari_olabilir': [], 'brand_sutunlari_olabilir': [],
        'price_sutunlari_olabilir': [], 'case_sutunlari_olabilir': [],
        'quantity_sutunlari_olabilir': [], 'pk_sutunlari_olabilir': []
    }
    maliyet_dict = {}
    ...
    for satir in satirlar:
        ...
        if a == 0 and '=' in satir:
            ...
            if key == 'upc': sutun_dictionary['upc_sutunlari_olabilir'] = vals
            elif key == 'brand': sutun_dictionary['brand_sutunlari_olabilir'] = vals
            elif key == 'price': sutun_dictionary['price_sutunlari_olabilir'] = vals
            elif key == 'case': sutun_dictionary['case_sutunlari_olabilir'] = vals
            elif key == 'quantity on hand': sutun_dictionary['quantity_sutunlari_olabilir'] = vals
            elif key == 'pk': sutun_dictionary['pk_sutunlari_olabilir'] = vals
...
def process_export(path, row_file, export_files, columns_dict, dataframe_dictionary):
    ...
    colrow = find_column(row_df, columns_dict['upc_sutunlari_olabilir'], f'{row_file} ham dosyası için UPC sütunu bulunamadı.')
    row_upcs = set(row_df[colrow].tolist())
    
    export_file = next((f for f in export_files if os.path.basename(f).split('-')[0] == row_code), None)
    if not export_file:
        raise ValueError(f"Eşleşen export dosyası bulunamadı: {row_code}")
        
    export_df = pd.read_excel(export_file, engine='openpyxl')
    colexp = find_column(export_df, columns_dict['upc_sutunlari_olabilir'], f'{export_file} export dosyası için UPC sütunu bulunamadı.')
    qtycol = find_column(export_df, columns_dict['quantity_sutunlari_olabilir'], f'{export_file} export dosyası için Quantity sütunu bulunamadı.')
    ...
    price_sutun = find_column(row_df, columns_dict['price_sutunlari_olabilir'], f'{row_file} ham dosyası için Price sütunu bulunamadı.')
    ...
    row_df.to_excel(save_path, index=False, sheet_name='export sonuc', engine='openpyxl')
    return row_df

def process_restock_logic(path, row_files, export_files, restock_files, islem, save_name, progress_callback=None):
    if not row_files:
    ...
    for i, file in enumerate(row_files):
        if progress_callback: progress_callback(f"Okunuyor ({i+1}/{len(row_files)}): {os.path.basename(file)}", 10)
        dataframe_dictionary[file] = pd.read_excel(file, engine='openpyxl')

    # 2. EXPORT
    if islem.get('export') == 1:
        if not export_files: raise ValueError("Export seçildi ancak export dosyaları eksik.")
        for i, row_file in enumerate(row_files):
            if progress_callback: progress_callback(f"Export işleniyor ({i+1}/{len(row_files)}): {os.path.basename(row_file)}", 30)
            dataframe_dictionary[row_file] = process_export(path, row_file, export_files, columns_dict, dataframe_dictionary)

    # 3. BİRBİRİNDEN DÜŞME TESPTİ
    if progress_callback: progress_callback("Silinecek UPC değerleri tespit ediliyor...", 50)
    remove_upc = {file: [] for file in row_files}
    for i, file in enumerate(row_files):
        this_df = dataframe_dictionary[file]
        upc_col = find_column(this_df, columns_dict['upc_sutunlari_olabilir'], f'{file} için UPC bulunamadı.')
        price_col = find_column(this_df, columns_dict['price_sutunlari_olabilir'], f'{file} için Price bulunamadı.')
        ...
        for next_file in row_files[i+1:]:
            ...
            n_upc_col = find_column(next_df, columns_dict['upc_sutunlari_olabilir'], f'{next_file} için UPC bulunamadı.')
            n_price_col = find_column(next_df, columns_dict['price_sutunlari_olabilir'], f'{next_file} için Price bulunamadı.')
            next_upc_dict = next_df.set_index(n_upc_col)[n_price_col].to_dict()
            
            for upc, price in this_upc_dict.items():
                if upc in next_upc_dict:
                    if price < next_upc_dict[upc]: remove_upc[next_file].append(upc)
                    elif price > next_upc_dict[upc]: remove_upc[file].append(upc)
                    else: remove_upc[next_file].append(upc)

    # 4. BİRBİRİNDEN DÜŞME UYGULAMA
    row_dataframe_dictionary = {}
    for i, file in enumerate(row_files):
        if progress_callback: progress_callback(f"UPC'ler siliniyor: {os.path.basename(file)}", 60)
        df = dataframe_dictionary[file]
        upc_col = find_column(df, columns_dict['upc_sutunlari_olabilir'], f'{file} için UPC bulunamadı.')
    ...
    if islem.get('restock') == 1:
        if not restock_files: raise ValueError("Restock (Ana) excel dosyası eksik.")
        if progress_callback: progress_callback("Restock birleştirmesi yapılıyor...", 70)
        ...
        main_upc_col = find_column(main_excel_df, columns_dict['upc_sutunlari_olabilir'], f'{main_excel} için UPC bulunamadı.')
        main_upc_list = main_excel_df[main_upc_col].tolist()
        main_pk_col = find_column(main_excel_df, columns_dict['pk_sutunlari_olabilir'], f'{main_excel} için PK bulunamadı.')
        main_pk_list = main_excel_df[main_pk_col].tolist()
        
        main_dict = {}
        for i, upc in enumerate(main_upc_list):
            main_dict[i] = {
                'upc': upc, 'brand': '#YOK', 'suplier': '#YOK', 'price': '#YOK', 
                'case': '#YOK', 'qtyonhand': '#YOK', 'PK': main_pk_list[i], 'maliyet': '#YOK'
            }
            
        for i, file in enumerate(row_files):
            row_upc_col = find_column(row_dataframe_dictionary[file], columns_dict['upc_sutunlari_olabilir'], 'UPC Yok')
            row_case_col = find_column(row_dataframe_dictionary[file], columns_dict['case_sutunlari_olabilir'], 'Case Yok')
            row_quantity_col = find_column(row_dataframe_dictionary[file], columns_dict['quantity_sutunlari_olabilir'], 'Quantity Yok')
            
            export_upc_col = find_column(dataframe_dictionary[file], columns_dict['upc_sutunlari_olabilir'], 'UPC Yok')
            export_price_col = find_column(dataframe_dictionary[file], columns_dict['price_sutunlari_olabilir'], 'Price Yok')
            export_brand_col = find_column(dataframe_dictionary[file], columns_dict['brand_sutunlari_olabilir'], 'Brand Yok')
            export_quantity_col = find_column(dataframe_dictionary[file], columns_dict['quantity_sutunlari_olabilir'], 'Quantity Yok')
            ...
            export_quantity_list = dataframe_dictionary[file][export_quantity_col].tolist()

            export_dict = {}
            for j, upc in enumerate(export_upc_list):
                export_dict[upc] = {'price': export_price_list[j], 'quantity': export_quantity_list[j], 'brand': export_brand_list[j]}
                
            row_dict = {}
            for j, upc in enumerate(row_upc_list):
                row_dict[upc] = {'case': row_case_list[j], 'quantity': row_quantity_list[j]}
        ...
        if progress_callback: progress_callback("Restock dosyası kaydediliyor...", 90)
        ...
        main_excel_df.to_excel(os.path.join(path, 'restock', f'{save_name}.xlsx'), index=False, sheet_name='restock', engine='openpyxl')

    if progress_callback: progress_callback("Tüm işlemler başarıyla tamamlandı!", 100)
