...
def validate_files(input_files, expected_extension):
    ...
    for file in input_files:
        ...
        if not file.lower().endswith(f".{expected_extension}"):
            raise ValueError(f"Hata: Geçersiz dosya tipi. Tüm dosyalar '{expected_extension}' olmalıdır -> {file}")

def process_conversion(input_files: list, output_folder: str, input_type: str, output_type: str, progress_callback=None) -> dict:
    if progress_callback: progress_callback("Aşama 1: Ön doğrulama yapılıyor...")
    ...
    if not read_func:
        raise ValueError(f"Mantıksal Hata: Desteklenmeyen girdi formatı -> {input_type}")

    processed_files = []
    for index, file in enumerate(input_files):
        filename = os.path.basename(file)
        if progress_callback: progress_callback(f"Aşama 2: İşleniyor ({index+1}/{len(input_files)}): {filename}")
        ...
        if output_type == 'csv':
            ...
            raise ValueError(f"Mantıksal Hata: Desteklenmeyen çıktı formatı -> {output_type}")
            
        processed_files.append(save_path)

    return {"status": "success", "message": f"{len(processed_files)} dosya başarıyla dönüştürüldü!", "output_path": target_dir}